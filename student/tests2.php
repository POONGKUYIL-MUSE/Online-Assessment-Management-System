...
<div class="content container">
  ...
  <tr align='center'>
    <th>Quiz</th><th>Type</th><th>No of Qns</th><th>Attempts</th>
  </tr>
  <?php
  $sql = "SELECT * FROM quizzes where status='active'";
  if($res = mysqli_query($conn,$sql)){
    while($row=mysqli_fetch_assoc($res)){
      echo "<tr align='center'>
      <td><a href='accept.php?quiz=".$row['quizname']."'>".$row['quizname']."</a></td>
      <td>".$row['type']."</td>
      <td>".$row['qnos']."</td>
      <td>".$row['attmpt']."</td>";

      // <href='launchtest.php?cquiz=".$row['quizname']."'>
    }
  }
  ?>
</table>
